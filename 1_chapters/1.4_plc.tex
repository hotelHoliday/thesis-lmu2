...
\section{Special Functions}
    ...
    \subsection{Automatic Control Source Recovery}
        ...
        Communication status of the Siemens \acrshort{hmi} is detected by a heartbeat signal within a status word while the Node-RED and LabVIEW program both utilise a watchdog timer. The Siemens heart beat is built into the configuration of the device while the watchdog timers for Node-RED and LabVIEW needed to be coded into the program, this can be seen within Appendix \HD{What is the Appendix number for the LabVIEW program and also the Node-RED program?}
